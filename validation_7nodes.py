# -*- coding: utf-8 -*-
"""
Created on Tue Feb 23 11:27:49 2021
...
def closest_to_one(lst):
    """Return the value in `lst` that is closest to 1."""
...
def main():
    ...
    for i in range(SIZE):
        for j in range(SIZE):
            norm_factor = max(node_degrees[i]["out"]["positive"],
                              node_degrees[j]["out"]["positive"])
            if norm_factor != 0:
                b_pos[i, j] = b_pos[i, j] / norm_factor
    '''
    ...
    for i in range(SIZE):
        for j in range(SIZE):
            norm_factor = max(node_degrees[i]["out"]["negative"],
                              node_degrees[j]["out"]["negative"])
            if norm_factor != 0:
                b_neg[i, j] = b_neg[i, j] / norm_factor
    '''
    ...
    for i in range(SIZE):
        for j in range(SIZE):
            norm_factor = max(node_degrees[i]["in"]["positive"],
                              node_degrees[j]["in"]["positive"])
            if norm_factor != 0:
                c_pos[i, j] = c_pos[i, j] / norm_factor
    '''
    ...
    for i in range(SIZE):
        for j in range(SIZE):
            norm_factor = max(node_degrees[i]["in"]["negative"],
                              node_degrees[j]["in"]["negative"])
            if norm_factor != 0:
                c_neg[i, j] = c_neg[i, j] / norm_factor
    '''
    ...
    for i in range(SIZE):
        for j in range(SIZE):
            b_list = [(1 + b_pos[i, j]) / (1 + b_neg[i, j]),
                      (1 + b_neg[i, j]) / (1 + b_pos[i, j])]
            balance_out[i, j] = closest_to_one(b_list)

            c_list = [(1 + c_pos[i, j]) / (1 + c_neg[i, j]),
                      (1 + c_neg[i, j]) / (1 + c_pos[i, j])]
            balance_in[i, j] = closest_to_one(c_list)

    #balance_in = np.round(balance_in, decimals=ACCURACY)
    #balance_out = np.round(balance_out, decimals=ACCURACY)

    print("Balance Out Matrix\n", balance_out, "\n")
    print("Balance In Matrix\n", balance_in, "\n")
    ...
    print("Similarity Out Matrix\n", sim_out, "\n")
    print("Similarity In Matrix\n", sim_in, "\n")
